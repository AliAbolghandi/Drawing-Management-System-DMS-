  ...
  try {
    ...
    try {
      ...
      if (Number(identity.recordset[0]?.IsIdentity) === 1) {
        const r = new sql.Request(tx); r.input('parentId',sql.Int,parentId); r.input('code',sql.NVarChar(100),code); r.input('name',sql.NVarChar(255),name); r.input('folderPath',sql.NVarChar(sql.MAX),folderPath); r.input('pathId',sql.Int,parent.PathID ?? null);
        result = await r.query(`INSERT INTO dbo.Nodes (ParentID,NodeCode,NodeName,IsActive,CreatedAt,UpdatedAt,FolderPath,PathID) OUTPUT INSERTED.* VALUES (@parentId,@code,@name,1,SYSUTCDATETIME(),SYSUTCDATETIME(),@folderPath,@pathId);`);
      } else {
        const idr = await new sql.Request(tx).query('SELECT ISNULL(MAX(NodeID),0)+1 AS NodeID FROM dbo.Nodes WITH (UPDLOCK,HOLDLOCK);');
        const r = new sql.Request(tx); r.input('nodeId',sql.Int,Number(idr.recordset[0].NodeID)); r.input('parentId',sql.Int,parentId); r.input('code',sql.NVarChar(100),code); r.input('name',sql.NVarChar(255),name); r.input('folderPath',sql.NVarChar(sql.MAX),folderPath); r.input('pathId',sql.Int,parent.PathID ?? null);
        result = await r.query(`INSERT INTO dbo.Nodes (NodeID,ParentID,NodeCode,NodeName,IsActive,CreatedAt,UpdatedAt,FolderPath,PathID) OUTPUT INSERTED.* VALUES (@nodeId,@parentId,@code,@name,1,SYSUTCDATETIME(),SYSUTCDATETIME(),@folderPath,@pathId);`);
      }
