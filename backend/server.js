...
async function computeSrscStatus() {
    ...
    const result = await sql.query(`
        ...
        WHERE N.FolderPath IS NOT NULL AND LTRIM(RTRIM(N.FolderPath)) <> ''
    `);
    const nodeIds = [];
    ...
    try {
        ...
        if (req.query.refresh !== '1' && !srscStatusCache) {
            getSrscStatus(false)
                ...
                .catch(err => console.error('SRSC status background scan failed:', err.message));
            return res.json({ nodeIds: [], count: 0, pending: true });
        }
        const nodeIds = await getSrscStatus(req.query.refresh === '1');
        res.json({ nodeIds, count: nodeIds.length, pending: false });
    } catch (err) {
        console.error('SRSC status scan failed:', err);
        res.status(500).json({ error: err.message });
    }
});
...
app.get('/api/node-folders/:nodeId', async (req, res) => {
    ...
    try {
        ...
        const folders = ALLOWED_SRSC_FOLDERS.map(name => {
            const fullPath = path.join(nodeRoot, name);
            try { return { name, exists: fs.existsSync(fullPath) && fs.statSync(fullPath).isDirectory() }; }
            catch (_) { return { name, exists: false }; }
        }).filter(f => f.exists);
